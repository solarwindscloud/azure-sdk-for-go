...
const fqdn = "github.com/Azure/azure-sdk-for-go/services/preview/machinelearning/mgmt/2018-11-19/services"
...
type AmlComputeProperties struct {
	...
	Errors *[]MachineLearningServiceError `json:"errors,omitempty"`
...
// ComputeNodesInformationModel ...
type ComputeNodesInformationModel struct {
	autorest.Response `json:"-"`
	Value             BasicComputeNodesInformation `json:"value,omitempty"`
}

// UnmarshalJSON is the custom unmarshaler for ComputeNodesInformationModel struct.
func (cnim *ComputeNodesInformationModel) UnmarshalJSON(body []byte) error {
	cni, err := unmarshalBasicComputeNodesInformation(body)
	if err != nil {
		return err
	}
	cnim.Value = cni
...
// ListUsagesByVMFamilyResult the List UsagesByVMFamily operation response.
type ListUsagesByVMFamilyResult struct {
	autorest.Response `json:"-"`
	// Value - READ-ONLY; The list of compute resource usages, by VM family, broken down by Workspace, and Cluster usage.
	Value *[]UsageByVMFamily `json:"value,omitempty"`
	// NextLink - READ-ONLY; The URI to fetch the next page of compute resource usage information.
	NextLink *string `json:"nextLink,omitempty"`
}

// ListUsagesByVMFamilyResultIterator provides access to a complete listing of UsageByVMFamily values.
type ListUsagesByVMFamilyResultIterator struct {
	i    int
	page ListUsagesByVMFamilyResultPage
}

// NextWithContext advances to the next value.  If there was an error making
// the request the iterator does not advance and the error is returned.
func (iter *ListUsagesByVMFamilyResultIterator) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ListUsagesByVMFamilyResultIterator.NextWithContext")
		defer func() {
			sc := -1
...
func (iter *ListUsagesByVMFamilyResultIterator) Next() error {
	return iter.NextWithContext(context.Background())
}

// NotDone returns true if the enumeration should be started or is not yet complete.
func (iter ListUsagesByVMFamilyResultIterator) NotDone() bool {
	return iter.page.NotDone() && iter.i < len(iter.page.Values())
}

// Response returns the raw server response from the last page request.
func (iter ListUsagesByVMFamilyResultIterator) Response() ListUsagesByVMFamilyResult {
	return iter.page.Response()
}

// Value returns the current value or a zero-initialized value if the
// iterator has advanced beyond the end of the collection.
func (iter ListUsagesByVMFamilyResultIterator) Value() UsageByVMFamily {
	if !iter.page.NotDone() {
		return UsageByVMFamily{}
	}
	return iter.page.Values()[iter.i]
}

// Creates a new instance of the ListUsagesByVMFamilyResultIterator type.
func NewListUsagesByVMFamilyResultIterator(page ListUsagesByVMFamilyResultPage) ListUsagesByVMFamilyResultIterator {
	return ListUsagesByVMFamilyResultIterator{page: page}
}

// IsEmpty returns true if the ListResult contains no values.
func (lubvfr ListUsagesByVMFamilyResult) IsEmpty() bool {
	return lubvfr.Value == nil || len(*lubvfr.Value) == 0
}

// listUsagesByVMFamilyResultPreparer prepares a request to retrieve the next set of results.
// It returns nil if no more results exist.
func (lubvfr ListUsagesByVMFamilyResult) listUsagesByVMFamilyResultPreparer(ctx context.Context) (*http.Request, error) {
	if lubvfr.NextLink == nil || len(to.String(lubvfr.NextLink)) < 1 {
		return nil, nil
	}
	return autorest.Prepare((&http.Request{}).WithContext(ctx),
		autorest.AsJSON(),
		autorest.AsGet(),
		autorest.WithBaseURL(to.String(lubvfr.NextLink)))
}

// ListUsagesByVMFamilyResultPage contains a page of UsageByVMFamily values.
type ListUsagesByVMFamilyResultPage struct {
	fn     func(context.Context, ListUsagesByVMFamilyResult) (ListUsagesByVMFamilyResult, error)
	lubvfr ListUsagesByVMFamilyResult
}

// NextWithContext advances to the next page of values.  If there was an error making
// the request the page does not advance and the error is returned.
func (page *ListUsagesByVMFamilyResultPage) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ListUsagesByVMFamilyResultPage.NextWithContext")
		defer func() {
			sc := -1
	...
	next, err := page.fn(ctx, page.lubvfr)
	if err != nil {
		return err
	}
	page.lubvfr = next
...
// the request the page does not advance and the error is returned.
// Deprecated: Use NextWithContext() instead.
func (page *ListUsagesByVMFamilyResultPage) Next() error {
	return page.NextWithContext(context.Background())
}

// NotDone returns true if the page enumeration should be started or is not yet complete.
func (page ListUsagesByVMFamilyResultPage) NotDone() bool {
	return !page.lubvfr.IsEmpty()
}

// Response returns the raw server response from the last page request.
func (page ListUsagesByVMFamilyResultPage) Response() ListUsagesByVMFamilyResult {
	return page.lubvfr
}

// Values returns the slice of values for the current page or nil if there are no values.
func (page ListUsagesByVMFamilyResultPage) Values() []UsageByVMFamily {
	if page.lubvfr.IsEmpty() {
		return nil
	}
	return *page.lubvfr.Value
}

// Creates a new instance of the ListUsagesByVMFamilyResultPage type.
func NewListUsagesByVMFamilyResultPage(getNextPage func(context.Context, ListUsagesByVMFamilyResult) (ListUsagesByVMFamilyResult, error)) ListUsagesByVMFamilyResultPage {
	return ListUsagesByVMFamilyResultPage{fn: getNextPage}
}
...
func (future *MachineLearningComputeUpdateFuture) Result(client MachineLearningComputeClient) (cr ComputeResource, err error) {
	...
	if err != nil {
		err = autorest.NewErrorWithError(err, "services.MachineLearningComputeUpdateFuture", "Result", future.Response(), "Polling failure")
		return
	}
	if !done {
		err = azure.NewAsyncOpIncompleteError("services.MachineLearningComputeUpdateFuture")
		return
	}
	sender := autorest.DecorateSender(client, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	if cr.Response.Response, err = future.GetResult(sender); err == nil && cr.Response.Response.StatusCode != http.StatusNoContent {
		...
		if err != nil {
			err = autorest.NewErrorWithError(err, "services.MachineLearningComputeUpdateFuture", "Result", cr.Response.Response, "Failure responding to request")
		}
	}
	return
}
...
// UsageByVMFamily describes Batch AI Resource Usage by VM Family, broken down by Workspace and Cluster
// usage
type UsageByVMFamily struct {
	// UsageByVMFamilyProperties - The properties associated with UsageByVMFamily
	*UsageByVMFamilyProperties `json:"properties,omitempty"`
	// Unit - READ-ONLY; An enum describing the unit of usage measurement. Possible values include: 'Count'
	Unit UsageUnit `json:"unit,omitempty"`
	// CurrentValue - READ-ONLY; The current usage of the resource.
	CurrentValue *int64 `json:"currentValue,omitempty"`
...
func (ubvf UsageByVMFamily) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]interface{})
	if ubvf.UsageByVMFamilyProperties != nil {
		objectMap["properties"] = ubvf.UsageByVMFamilyProperties
	}
	return json.Marshal(objectMap)
}

// UnmarshalJSON is the custom unmarshaler for UsageByVMFamily struct.
func (ubvf *UsageByVMFamily) UnmarshalJSON(body []byte) error {
	var m map[string]*json.RawMessage
	err := json.Unmarshal(body, &m)
	if err != nil {
		return err
	...
	for k, v := range m {
		...
		case "properties":
			if v != nil {
				var usageByVMFamilyProperties UsageByVMFamilyProperties
				err = json.Unmarshal(*v, &usageByVMFamilyProperties)
				if err != nil {
					return err
				}
				ubvf.UsageByVMFamilyProperties = &usageByVMFamilyProperties
			}
		case "unit":
			if v != nil {
				var unit UsageUnit
		...
		case "currentValue":
			if v != nil {
				...
				err = json.Unmarshal(*v, &currentValue)
				if err != nil {
					return err
				}
				ubvf.CurrentValue = &currentValue
			}
		case "limit":
			if v != nil {
				var limit int64
...
// UsageByVMFamilyProperties properties of UsageByVMFamily definition, an extension of Usage definition
type UsageByVMFamilyProperties struct {
	// ResourceGroupName - READ-ONLY; The name of the resource group this resource type belongs to
	ResourceGroupName *string `json:"resourceGroupName,omitempty"`
	// ResourceType - READ-ONLY; The type of the resource, whether its VM family, workspace name, or cluster
	ResourceType *string `json:"resourceType,omitempty"`
	// UsageBreakdown - READ-ONLY; The breakdown of usage by Workspace or Cluster
	UsageBreakdown *[]UsageByVMFamily `json:"usageBreakdown,omitempty"`
}
