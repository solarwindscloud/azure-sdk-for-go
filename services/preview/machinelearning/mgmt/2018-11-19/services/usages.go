...
func (client UsagesClient) List(ctx context.Context, location string) (result ListUsagesResultPage, err error) {
	...
	if err := validation.Validate([]validation.Validation{
		...
		return result, validation.NewError("services.UsagesClient", "List", err.Error())
	}

	result.fn = client.listNextResults
	req, err := client.ListPreparer(ctx, location)
	if err != nil {
		err = autorest.NewErrorWithError(err, "services.UsagesClient", "List", nil, "Failure preparing request")
		return
	}
	...
	if err != nil {
		...
		err = autorest.NewErrorWithError(err, "services.UsagesClient", "List", resp, "Failure sending request")
		return
	}

	result.lur, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "services.UsagesClient", "List", resp, "Failure responding to request")
	}
...
func (client UsagesClient) listNextResults(ctx context.Context, lastResults ListUsagesResult) (result ListUsagesResult, err error) {
	...
	if err != nil {
		return result, autorest.NewErrorWithError(err, "services.UsagesClient", "listNextResults", nil, "Failure preparing next results request")
	}
	if req == nil {
		return
	}
	...
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		return result, autorest.NewErrorWithError(err, "services.UsagesClient", "listNextResults", resp, "Failure sending next results request")
	}
	result, err = client.ListResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "services.UsagesClient", "listNextResults", resp, "Failure responding to next results request")
	}
	return
}
